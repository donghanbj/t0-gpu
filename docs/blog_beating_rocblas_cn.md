...
我在做一个自定义的神经网络训练系统。正常路径是：装 ROCm（2+ GB），用 HIP，调 rocBLAS，完事。但我遇到了三个问题：

1. **调度延迟**：HIP 的同步 kernel launch 要 ~20μs。训练时每一步要启动几百个小内核，这个开销累积起来就是几秒钟。
2. **依赖地狱**：ROCm 版本经常互相不兼容。我的 Ubuntu 24.04 需要特定的 ROCm 7.x 构建，驱动/运行时的版本矩阵简直是噩梦。
3. **黑盒**：GPU 挂起时（裸金属开发中经常发生），HIP 什么信息都不给。用 KFD 至少能看到 AQL 队列状态。
