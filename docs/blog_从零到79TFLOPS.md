...
> *一周密集开发实录 — 从 GPU 硬挂 20+ 次到稳定输出 79.2 TFLOPS*
...
这篇文章记录了 T0-GPU 编译器在 2026 年 3 月 23 日至 29 日这一周里，从"GPU 硬挂 20+ 次"到"稳定 79.2 TFLOPS"的完整历程。每一个 Bug 都是用硬挂和强制重启换来的教训。
...
这一周里累计硬挂 **20+ 次**。每次重启大约需要 3-5 分钟（等 GPU 恢复 + Linux 重新初始化 KFD），光是重启就浪费了好几个小时。
...
经过两天的密集调试，9 个根因 Bug 被逐一消灭：

| 指标 | 3/25 开始 | 3/26 结束 |
|------|-----------|-----------|
| GPU 硬挂 | 频繁 (20+ 次累计) | **0 次** |
| 测试通过率 | 0/9 | **10/10** |
...
| 3/25 AM | GPU 频繁硬挂 | LICM + raw_asm + KFD 三层防御 |
